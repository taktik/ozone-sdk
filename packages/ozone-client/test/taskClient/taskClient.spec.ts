...
describe('OzoneClient', () => {
	...
	describe('taskClient', () => {
		describe('TaskHandlerImpl', () => {
			...
			describe('onProgress', () => {
				it('should be called at each update', () => new Promise<void>((done) => {
			...
			describe('onFinish', () => {
				it('should be called when task is done', () => new Promise<void>((done) => {
		...
		describe('TaskHandlerImpl error mangement', () => {
			beforeEach(() => {
				...
				server.respondWith(
					'GET',
					/\/rest\/v3\/task\/wait\/task-id-with-error\/\d+$/,
					xhr => {
						xhr.respond(200,
							{ 'Content-Type': 'application/json' },
							JSON.stringify(responses[responseIndex++]))
			...
			describe('onError', () => {
				it('should be called when task has an error', () => new Promise<void>((done) => {
			...
			describe('stopWaiting', () => {
				it('should cancel ozone call and call onError', () => new Promise<void>((done) => {
				...
				it('should cancel ozone call and reject waitResult', () => new Promise<void>((done) => {
					const handler = new TaskHandlerImpl('task-to-cancel', client, `http://my.ozone.domain/ozone`, { pollInterval: 10 })
					handler.waitResult
						.catch((taskExecution) => {
							expect(taskExecution).to.deep.equal({ error: 'Wait for task canceled' })
