	...
	describe('permissionClient', () => {
		describe('bulkGetPermissions', () => {
			it('shoud send POST request permission', async () => {
				server.respondWith('POST', /\/rest\/v3\/items\/bulkGetPermissions\?fields=name$/, [
					200,
				...
				assert.deepEqual(data.get('item_id_1')!.grant, {
					id: 'item_id_1',
					grants: ['FIELD_EDIT', 'FIELD_VIEW'],
					fieldGrants: { name: ['FIELD_EDIT', 'FIELD_VIEW'] },
				} as any)
